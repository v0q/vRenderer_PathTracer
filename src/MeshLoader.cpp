  ...
  for(unsigned int i = 0; i < scene->mNumMeshes; ++i)
  {
    ...
    unsigned int numFaces = mesh->mNumFaces;
		...
		for(unsigned int j = 0; j < numVerts; ++j)
		{
			...
			vertices[j] = vFloat3(vert.x + offset, vert.y + offset/2., vert.z + offset);
		}
		...
		for(unsigned int j = 0; j < numFaces; ++j)
		{
			...
			vFloat3 e1 = vertices[triangles[j].m_indices[1]] - vertices[triangles[j].m_indices[0]];
			vFloat3 e2 = vertices[triangles[j].m_indices[2]] - vertices[triangles[j].m_indices[1]];
			vFloat3 e3 = vertices[triangles[j].m_indices[0]] - vertices[triangles[j].m_indices[2]];
